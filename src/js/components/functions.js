...
export const bodyLock = (delay = 500) => {
	...
	if (bodyLockStatus) {
		...
		document.documentElement.classList.add('overflowLock');
...
export const bodyUnlock = (delay = 500) => {
	...
	if (bodyLockStatus) {
		...
		setTimeout(() => {
			...
			document.documentElement.classList.remove('overflowLock');
		}, delay);
		bodyLockStatus = false;
		setTimeout(() => {
			bodyLockStatus = true;
...
export const bodyLockToggle = (delay = 500) => {
	if (document.documentElement.classList.contains('overflowLock')) {
		bodyUnlock(delay);
	} else {
		bodyLock(delay);
	}
